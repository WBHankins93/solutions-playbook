## Pre-Call Prep with Sales

### Context

Before any significant customer call (discovery, demo, POC kickoff). Align on
roles and context so you're not walking in blind.

### Quick Sync Agenda (15 min max)

1. **Customer context**
   - What's the deal status? What stage?
   - What's the revenue potential? Timeline pressure?
   - Any politics or sensitivities?

2. **Attendees**
   - Who's on the call from their side?
   - Who's the decision maker? Technical evaluator?
   - Anyone to be cautious around?

3. **Roles on our side**
   - Who's leading the call?
   - Who's taking notes?
   - Who handles what questions?

4. **Desired outcome**
   - What do we want to walk away with?
   - What's the next step we're aiming for?

5. **Landmines**
   - Features we don't have that might come up?
   - Competitor mentions?
   - Pricing concerns?

### Links

- [Discovery Call](../pre-sales/discovery.md)
- [Demo](../pre-sales/demo.md)
- [Post-call debrief](post-call-debrief.md)

